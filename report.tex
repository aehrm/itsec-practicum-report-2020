...
While many Bitcoin users can rely on \textquote{simplified payment verification} and/or blockchain indexing servers to avoid downloading a full copy of the blockchain, full nodes – maintaining a complete up-to-date copy of the blockchain – are required in the network to independently verify the transaction ledger \cite{nakamoto_bitcoin_2008}, cf.~\cite[Chap.~8–9]{antonopoulos_mastering_2017}.
...
Nevertheless, we can identify a minimal set of general principles that can be considered defining for Bitcoin, that is \textquote{even if all Bitcoin users decide to adopt any of these changes, the resulting cryptocurrency can no longer be considered \textquote*{Bitcoin} because it has diverged too much from the original design} \cite{bitcoinwiki_prohibited_2016}.
...
Independently, Matzutt et al.\@ \cite{matzutt_thwarting_2018} discussed three approaches as countermeasure:\@ (a) heuristic content detectors filtering out problematic data, (b) increasing mandatory transaction fee penalizing large transactions, and (c) \textquote{self-verifying account identifiers}, similarly exploiting the one-way property of hash functions.
...
Promising results were obtained by Florian et al.\@ \cite{florian_erasing_2019}, proposing a lightweight approach using \textquote{functionality-preserving local erasure}.
...
Nevertheless, without loss of generality we can abstract away from that specific implementation and focus on \textquote*{full} preimages as defined in previous paragraph, since Seeg's approach identically relies on the one-way property.
...
\begin{table}[t]
    ...
    \begin{tabular}{lrrrS[table-format=0.8,table-alignment=right]rS[table-format=2.1e-1,table-alignment=right]}
        ...
        \midrule
        1-of-3 P2MS  & 105 B & 32 B& 600 &   0.00617780 &1029.63 & 10.3e-2\cr
        1-of-3 P2FMS & 201 B & 65 B& 600 &   0.01059380 &1765.63 & 17.7e-2\cr
        \bottomrule
    \end{tabular}
