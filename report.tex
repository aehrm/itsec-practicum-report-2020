...
\end{displayquote}
While most such inclusions of off-topic data in practice are negleglibe in size, more sophisticated methods allowed actors to include entire files such as images and PDF files.
...
Hence from a theoretical perspective, when given a concensus on which methods are considered so controversial that they are rejected, we can determine a maximal effectivity of any countermeasure that is considered acceptable in the sense of the given concensus.

Due to the multitude of different interest groups and actors that constitute Bitcoin's community, a precise concensus of the approved methods available when designing an effective countermeasure seems unlikely.
...
the computational resources neccessary for that specific brute-force circumvention serve as a sensible upper bound for the strength of the countermeasure.

Having established this circumvention, we focus on the main part of the paper.
As first result, a stochastic model of expected inclusion cost, in combination with a sensible estimate of the currently availiable computing efficiency on the cryptographic primitives, allows us to assess the effectivity of the proposed countermeasure, and in turn, any countermeasure adhering the aforementioned principles.
Furthermore, as secondary result, we assess the impact on variations in bitcoin price and availiable computing efficiency.
...
and this countermeasure approriately takes respective signatures $\sigma$ as such proof of control.
Addresses that withstand the proposed test hence are precicely the addresses that can be generated by honest participants of the network, who should not be penalized by any countermeasure.
...
Hence, it seems unlikely that modifications on the interations could increase performance.
...
We further observe that the preimage loop can easily be parallelilized on multiple cores and computers, especially allowing efficient implementations on GPUs.
